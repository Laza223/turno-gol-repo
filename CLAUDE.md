...
- **No-show = deuda (cambio #5, modelo ATC)**: marcar no-show captura la seña pagada (`deposit_status='captured'`) y suma `price_snapshot − seña` a `player_tenant_relationships.balance`; el jugador queda bloqueado para reservar online en ese complejo hasta saldarla. Se eliminó el ban temporal por días (`no_show_penalty` ya no existe en settings, migr. 034); los bans manuales (`tenant_player_bans`) siguen para otros motivos. Lógica en `handleNoShow` (`src/modules/bookings/booking.cancellation.ts`), incremento atómico vía `addNoShowDebt` (definido en `src/modules/relationships/ptr.service.ts`).
- **Abonados sin saldo a favor (REVERTIDO, 2026-07-10)**: el sistema de crédito/saldo a favor copiado de ATC (`abonados.credit_balance`, `bookings.credit_applied`, `CashFlow` categoría `abonado_payment`) fue eliminado (migr. 042 revierte lo introducido en 033) — modelo ATC descartado para fútbol. El abonado NO tiene saldo a favor ni precio mensual: solo `price_per_session` (precio por sesión). Distinto de `player_tenant_relationships.balance` (deuda por no-show), que se conserva intacto.
- **Módulo Jugadores** (`/jugadores`, cambio #9): vista admin de jugadores vinculados al complejo (vía `player_tenant_relationships`, NUNCA guests). Ficha = stats + Deudas (cobrar `balance` → reduce PTR.balance + CashFlow) + Abonados + historial. Protegido con `requireOperatorStaff()` (admin + manager).
