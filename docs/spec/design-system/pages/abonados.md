...
día/horario todas las semanas): alta, reactivación y baja. A diferencia de Reservas, **no
tiene vista de detalle** (`/abonados/[id]`) — el listado + los diálogos de acción (reactivar/
cancelar) son toda la superficie de la feature. (2026-09-15: "Pausar" se retiró del producto —
saltear una fecha puntual se hace cancelando ese turno desde la Grilla, sin dar de baja el fijo.)
...
**2026-09-15**: `paused` ya no es alcanzable desde la UI ("Pausar" se retiró del producto), pero
el badge se conserva — hay filas `paused` de antes de ese cambio que solo pueden salir de ese
estado vía Reactivar.
...
`/abonados/[id]` — las acciones (Reactivar/Cancelar) ya viven inline en la fila vía
...
dark:text-amber-400` (hue warning consistente con el resto de la app — mismo par usado en
  ...
  "Cancelar" ya usaba `text-destructive` — sin cambios. (2026-09-15: el botón "Pausar" en sí se
  retiró del producto; el resto de esta migración de color sigue vigente para "Reactivar" y
  "Cancelar".)
...
5. `/abonados/nuevo` (wizard de alta) no se tocó — mismo criterio que Reservas no tocó `/grilla`
   ...
   corresponde.
6. **Moot desde 2026-09-15** (el link "Pausar" que medía este ítem se retiró del producto):
   `text-amber-700` daba ≈5,02:1 sobre `--card` — pasaba AA pero era un valor que MASTER §2.4 no
   tiene verificado en su tabla (solo mide `amber-600`/`amber-950` para fills). Se deja constancia
   por si `amber-700`/`amber-800` reaparece en otro control.

## §8 Contratos de test

- Unit `abonados-list.test.tsx`: los textos de badge ('Activo'/'Pausado'/'Cancelado') y los
  `getByRole('button', {name: 'Cancelar turno fijo'|'Reactivar'})` siguen fijos por el test
  (2026-09-15: "Pausar" se eliminó de la fila — el test agrega una aserción de regresión de que
  ya no aparece en una fila `active`).
