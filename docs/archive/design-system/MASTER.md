...
- Ítems bloqueados por rol (manager) muestran candado + tooltip "Solo el dueño" (§7.4) — no desaparecen (el encargado entiende el sistema completo). En el riel el trigger es un `button` con `aria-disabled`, no un `span`: el tooltip necesita foco y `disabled` lo mataría junto con el tooltip. Hoy lo ve el manager desde 2026-09-19 (es la pantalla del mostrador). Canchas y Métricas, que son solo del dueño, se ocultan del riel para el manager en vez de bloquearse; el candado es hoy solo de Configuración (pendiente de unificar).
