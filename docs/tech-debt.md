...
## ~~No hay forma de ofrecerle un plan a un solo complejo~~ — RESUELTA 2026-09-01

**Qué era**: `plans` sólo tenía `is_active` para controlar visibilidad — un plan estaba prendido para todos o apagado para todos. El workaround era prenderlo, suscribir y apagarlo, con dos agujeros: durante esa ventana cualquier complejo que abriera `/settings/facturacion` veía el plan interno, y **reactivar** una suscripción de un plan apagado fallaba con `PlanNotFoundError`.

**Cómo se resolvió**: migración `083`, columna nullable `plans.owner_tenant_id` (NULL = público) más índice parcial. `is_active` NO cambia de significado: sigue siendo el interruptor de encendido y la columna nueva es el alcance de visibilidad. Separarlos es lo que arregla la reactivación de rebote — un plan privado ya no necesita apagarse para dejar de ofrecerse, así que sigue activo y `loadPlan` lo encuentra.

Filtro aplicado en los tres lugares que leen el catálogo: `loadPlan` ([billing.service.ts](src/modules/billing/billing.service.ts), con el `tenantId` cableado desde sus 5 llamadores — `subscribe`, `upgrade`, `handleUpgradeApproved`, `downgrade`, `reactivate`), `listActivePlans` del complejo, y la gemela del panel ([super-admin/tenants.service.ts](src/modules/super-admin/tenants.service.ts)), que recibe un `forTenantId` opcional: sin argumento lista todo (el filtro de la lista de complejos, donde SuperAdmin ve la plataforma entera y no se está ofreciendo nada) y con argumento acota a públicos + el privado de ese complejo (la ficha, donde sí se ofrece).

**Hallazgo del camino, y es el que más costaba**: la versión con `FOREIGN KEY` a `tenants(id)` **hubiera roto todos los tests de billing en CI**, sin que nadie pudiera verlo localmente. Una FK convierte a `plans` en dependiente de `tenants`, y el `TRUNCATE tenants ... CASCADE` de `cleanupAll` (el harness de integración) habría truncado también `plans` — cuyas filas se siembran una única vez en la migración `007` y por lo tanto no vuelven. La columna quedó **sin FK a propósito**, con el motivo escrito en la cabecera de la migración. Lo que se pierde es poco: los complejos no se borran duro (soft-delete + anonimización), así que la FK protegía contra algo que el sistema no hace, y un `owner_tenant_id` colgado sólo vuelve el plan invisible para todos, que es el lado seguro del error.

**Tests**: `tests/unit/billing-private-plans-scoping.test.ts` (mira el SQL emitido, corre sin Docker, control positivo corrido) + `tests/integration/billing-private-plans.test.ts` (mira el resultado real contra Postgres, corre en CI). Los dos hacen falta porque `plans` es global y **sin RLS**: el filtro de la consulta es la única barrera.

**Pendiente para Lazar**: aplicar la migración 083 (`db:migrate` está denegado para los agentes).
