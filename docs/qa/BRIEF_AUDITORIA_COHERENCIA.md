...
| 8 | Esta misma fila, en su redacción original: "`settings/canchas/page.tsx:18` usa `requireOperatorStaff`... el manager entra por URL directa... Excepción real y no documentada" | **CORREGIDO (H163, 2026-09-10): no hay excepción.** `settings/layout.tsx:10` (`requireAdminStaff()`) bloquea las 6 pestañas por igual, Canchas incluida, ANTES de que la página se renderice — el `requireOperatorStaff` de `settings/canchas/page.tsx:18` nunca llega a correr. Coincide con §4 fila 8 de este mismo brief, que ya lo tenía bien ("código muerto"); esta fila de §14 fue la que quedó desactualizada | "El manager NO accede a Configuración" (CLAUDE.md) era correcto. El error estaba acá, no en CLAUDE.md — una lectura de `settings/canchas/page.tsx` sin seguir la cadena hasta el layout. Ya corregido en CLAUDE.md (bullet `staff_role`) |
