...
- [ ] Riesgo aceptado explícitamente: **NO hay kill switch implementado en código** (`DISABLE_PUBLIC_BOOKING` / `DISABLE_MP_PAYMENTS` / `FORCE_MANUAL_CONFIRMATION` no existen en `src/` — ver §5.2). El único lever disponible ante un incidente es rollback de Vercel (§3) o un fix manual en DB. Riesgo aceptado por: _[completar — quién]_.
