...
4. ~~**`updateProduct` stock (BK7):**~~ → **RESUELTO 2026-07-26: opción (a).** El form de catálogo deja de tocar stock; `updateProduct` lockea la fila y solo acepta cambio de MODO (null↔número) o no-op, rechazando número→otro número con `StockNotEditableFromCatalogError`. Las correcciones van por reposición/merma/ajuste, con rastro en el ledger. 5 tests nuevos (incluye el escenario del bug: venta concurrente durante la edición). Original: (a) separar edición de catálogo del ajuste de stock — el form de edición deja de tocar stock, correcciones solo por `adjustStock` con ledger (recomendada; cambia UX menor) vs (b) lock + registrar delta en ledger conservando el campo en el form.
5. ~~**Condición temporal de no-show:**~~ → **RESUELTO 2026-07-26: alinear el código a doc6.** `markNoShow` exige `ends_at` pasado (instante físico, no `date`/`time_end` reconstruido) en vez de `starts_at`; `BookingNotYetStartedError` → `NoShowNotYetEndedError` con mensaje nuevo. Criterio: marcar ausente captura la seña y puede disparar softban de 14 días, así que el estándar tiene que ser objetivo ("no vino en toda la hora"); la fricción es mínima porque quedan la ventana directa hasta el auto-complete (30 min) y la corrección de 24h. doc6 §3 ya especificaba `time_end` — no hizo falta tocar el spec. Original: ¿código pasa a exigir `time_end` (alinear a doc6) o doc6 se actualiza a `time_start` (permite marcar durante el turno)?
