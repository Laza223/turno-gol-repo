import { Banknote, Building2, Hourglass, Inbox, ListChecks, UserPlus, Webhook } from 'lucide-react'
...
/**
 * Cuerpo del dashboard global: 4 KPIs, tenants por estado, trials por vencer,
 * signups recientes, colas pg-boss y webhooks MP. Presentacional puro — el
 * fetch (`getDashboardData`) vive en la page; `PageHeader` también queda en
 * la page porque no depende de `data`.
 */
export function SuperAdminDashboardView({ data }: { data: DashboardData }) {
  const now = new Date()

  const totalTenants = Object.values(data.tenantsByStatus).reduce((acc, n) => acc + n, 0)
