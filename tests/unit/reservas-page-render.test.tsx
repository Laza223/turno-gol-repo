// @vitest-environment happy-dom
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { render, screen, within } from '@testing-library/react'
import type { ReservaListRow } from '@/app/(admin)/reservas/queries'
import { formatArs } from '@/lib/format'

// B10 — la page pasó a `requireOperatorStaff()`, que además del tenant lee el rol
// contra `tenant_staff_members`. Se mockea el guard, no las dos funciones que
// usaba antes por separado.
vi.mock('@/modules/staff/guards', () => ({
  requireOperatorStaff: vi.fn(async () => ({
    ok: true,
    user: { type: 'staff', staffUserId: 'staff-1' },
    role: 'admin',
    tenant: { id: 'tenant-1' },
  })),
}))
vi.mock('@/shared/db/client', () => ({
  withTenantContext: vi.fn(async (_id: string, cb: (tx: unknown) => unknown) => cb({})),
}))
// H110 — la page pide las canchas del tenant (filtro por cancha) dentro del
// mismo `withTenantContext`; sin este mock, `listCourts`
// real corre contra el `tx` de mentira de arriba y explota.
const courtsMock = vi.fn(async (): Promise<Array<{ id: string; name: string }>> => [])
vi.mock('@/modules/courts/court.service', () => ({
  listCourts: (...args: unknown[]) => courtsMock(...(args as [])),
}))
// `artTodayStr` fijo para poder aserter "Hoy"/"Mañana"/"Ayer"; `addDays` real
// (es pura) para no tener que reimplementar la aritmética acá.
vi.mock('@/shared/dates/art', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/shared/dates/art')>()
  return { ...actual, artTodayStr: vi.fn(() => '2026-06-12') }
})
const replaceMock = vi.fn()
// `ReservasHeaderBar` (client) lee `useSearchParams()` para preservar el
// resto de los params al pegar `router.replace` — en producción es la MISMA
// URL que ve el server. Acá hay que sincronizarlo a mano por test (mismo
// patrón que tenía `reservas-toolbar.test.tsx`).
let searchParamsInit = ''
vi.mock('next/navigation', () => ({
  redirect: vi.fn(() => {
    throw new Error('redirect llamado')
  }),
  useRouter: () => ({ replace: replaceMock }),
  usePathname: () => '/reservas',
  useSearchParams: () => new URLSearchParams(searchParamsInit),
}))

const listMock = vi.fn(async (): Promise<ReservaListRow[]> => [])
const countsMock = vi.fn(async (): Promise<Record<string, number>> => ({}))
/**
 * Cobros de mostrador por turno. La page los usa para derivar `pending` y
 * la plata de cada fila (`agendaMoneyCell`); con el Map vacío el saldo sale
 * del price_snapshot menos la seña, que es lo que quieren estos casos.
 */
const chargesMock = vi.fn(async (): Promise<Map<string, number>> => new Map())
/** `hasMore` de la página actual — mutable para poder renderizar el paginador. */
const hasMore = { value: false }
vi.mock('@/app/(admin)/reservas/queries', () => ({
  // El seam sigue devolviendo un array crudo para que cada caso arme su lista
  // con `listMock.mockResolvedValue([...])`; la envoltura de página (B10) se
  // agrega acá una sola vez.
  listTenantBookings: async (...args: unknown[]) => ({
    rows: await listMock(...(args as [])),
    hasMore: hasMore.value,
  }),
  RESERVAS_PAGE_SIZE: 50,
  countTenantBookingsByStatus: (...args: unknown[]) => countsMock(...(args as [])),
  sumBookingChargesByBooking: (...args: unknown[]) => chargesMock(...(args as [])),
}))

import ReservasPage from '@/app/(admin)/reservas/(list)/page'

function row(overrides: Partial<ReservaListRow>): ReservaListRow {
  return {
    id: 'b1',
    date: '2026-06-12',
    timeStart: '14:00:00',
    timeEnd: '15:00:00',
    status: 'confirmed',
    type: 'spontaneous',
    courtName: 'Cancha 1',
    playerName: null,
    guestName: 'Juan Pérez',
    phone: null,
    priceSnapshot: 2000000,
    depositAmount: 500000,
    depositStatus: 'paid',
    paymentMethod: 'cash',
    ...overrides,
  }
}

beforeEach(() => {
  vi.clearAllMocks()
  listMock.mockResolvedValue([])
  countsMock.mockResolvedValue({})
  courtsMock.mockResolvedValue([])
  hasMore.value = false
  searchParamsInit = ''
})

describe('ReservasPage — render', () => {
  it('sr-only h1 "Agenda": sin PageHeader, el nombre de la pantalla lo da GrillaTabs', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({}) }))
    expect(screen.getByRole('heading', { level: 1, name: 'Agenda' })).toBeTruthy()
  })

  it('agrupa por día operativo con encabezado "Hoy · …", sin contador por página', async () => {
    listMock.mockResolvedValue([
      row({ id: 'b1', date: '2026-06-12', timeStart: '14:00:00', timeEnd: '15:00:00' }),
      row({
        id: 'b2',
        date: '2026-06-12',
        timeStart: '16:00:00',
        timeEnd: '17:00:00',
        guestName: 'Ana López',
      }),
      row({ id: 'b3', date: '2026-06-13', timeStart: '09:00:00', timeEnd: '10:00:00' }),
    ])
    countsMock.mockResolvedValue({ confirmed: 3 })

    render(await ReservasPage({ searchParams: Promise.resolve({}) }))

    const secciones = screen.getAllByRole('region')
    expect(secciones).toHaveLength(2)
    const hoy = screen.getByRole('region', { name: /^Hoy · /i })
    expect(within(hoy).getAllByRole('article')).toHaveLength(2)
    // Sin "N turnos" por grupo: contaba solo la página de 50 (el total real va en el paginador).
    expect(within(hoy).queryByText(/\d+ turnos?$/)).toBeNull()
    const manana = screen.getByRole('region', { name: /^Mañana · /i })
    expect(within(manana).getAllByRole('article')).toHaveLength(1)
  })

  it('hoy pagina igual que cualquier otro día: la query recibe la página', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({ pagina: '2' }) }))
    expect(listMock).toHaveBeenCalledWith(
      'tenant-1',
      expect.objectContaining({ scope: 'proximos' }),
      expect.anything(),
      1,
    )
  })

  it('cada reserva es un article con aria-label descriptivo (plata incluida)', async () => {
    courtsMock.mockResolvedValue([{ id: 'c1', name: 'Cancha 1' }])
    listMock.mockResolvedValue([row({})])
    render(await ReservasPage({ searchParams: Promise.resolve({}) }))

    // La page deriva `pending`/`totalPaid` con `summarizeBookingCharges` (seña
    // paga 500.000 de un precio de 2.000.000, sin cobros de mostrador):
    // `agendaMoneyCell` lo lee como "Falta $X" del saldo, no el precio entero.
    expect(
      screen.getByRole('article', {
        name: `Turno 14:00–15:00, Cancha 1, Juan Pérez, Falta ${formatArs(1500000)}`,
      }),
    ).toBeTruthy()
  })

  it('la búsqueda se pasa a la query junto al scope', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({ q: '  maría  ' }) }))
    expect(listMock).toHaveBeenCalledWith(
      'tenant-1',
      expect.objectContaining({ scope: 'proximos', q: 'maría' }),
      expect.anything(),
      0,
    )
    expect(countsMock).toHaveBeenCalledWith(
      'tenant-1',
      expect.objectContaining({ scope: 'proximos', q: 'maría' }),
      expect.anything(),
    )
  })

  it('un ?vista viejo (bookmark/compartido) se ignora en silencio', async () => {
    courtsMock.mockResolvedValue([{ id: 'c1', name: 'Cancha 1' }])
    listMock.mockResolvedValue([row({})])
    render(await ReservasPage({ searchParams: Promise.resolve({ vista: 'compacta' } as never) }))
    // No revienta y la fila se ve igual que sin el param. Dos copias en el DOM
    // (renglón angosto + ancho, una oculta por CSS): `getAllByText`.
    expect(screen.getAllByText('Juan Pérez')[0]).toBeTruthy()
  })
})

describe('ReservasPage — vacío', () => {
  it('sin resultados y SIN búsqueda: ofrece "Cargar una reserva"', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({}) }))
    const link = screen.getByRole('link', { name: /Cargar una reserva/ })
    expect(link.getAttribute('href')).toBe('/grilla')
  })

  it('sin resultados CON búsqueda: ofrece buscar en el otro segmento', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({ q: 'tomás' }) }))
    const link = screen.getByRole('link', { name: /Buscar en Pasados/ })
    expect(link.getAttribute('href')).toBe('/reservas?dia=pasados&q=tom%C3%A1s')
  })

  it('sin resultados CON búsqueda en Pasados: ofrece volver a Próximos', async () => {
    render(await ReservasPage({ searchParams: Promise.resolve({ dia: 'pasados', q: 'tomás' }) }))
    const link = screen.getByRole('link', { name: /Buscar en Próximos/ })
    expect(link.getAttribute('href')).toBe('/reservas?q=tom%C3%A1s')
  })
})

/**
 * B10 — la UI que mentía: el chip "Todos"/pager y el subtítulo salían de un
 * COUNT sin techo mientras la lista se cortaba en 200 sin decirlo.
 */
describe('ReservasPage — paginación', () => {
  it('sin páginas siguientes NO muestra paginador (ruido en el caso normal)', async () => {
    listMock.mockResolvedValue([row({})])
    countsMock.mockResolvedValue({ confirmed: 1 })

    render(await ReservasPage({ searchParams: Promise.resolve({}) }))

    expect(screen.queryByRole('navigation', { name: /Paginación/ })).toBeNull()
  })

  it('con más páginas dice qué rango se está viendo y sobre qué total', async () => {
    hasMore.value = true
    listMock.mockResolvedValue([row({ id: 'b1' }), row({ id: 'b2' })])
    countsMock.mockResolvedValue({ confirmed: 740 })

    render(await ReservasPage({ searchParams: Promise.resolve({ dia: 'pasados' }) }))

    const aviso = screen.getByRole('status')
    expect(aviso.textContent).toContain('1–2')
    expect(aviso.textContent).toContain('740')
  })

  it('el link "Siguientes" preserva scope, filtro y búsqueda', async () => {
    hasMore.value = true
    listMock.mockResolvedValue([row({})])
    countsMock.mockResolvedValue({ no_show: 740 })

    render(
      await ReservasPage({
        searchParams: Promise.resolve({ dia: 'pasados', status: 'no_show', q: 'juan' }),
      }),
    )

    const pager = screen.getByRole('navigation', { name: /Paginación/ })
    expect(
      within(pager)
        .getByRole('link', { name: /Siguientes/ })
        .getAttribute('href'),
    ).toBe('/reservas?dia=pasados&status=no_show&q=juan&pagina=2')
    // En la página 1 no hay "Anteriores".
    expect(within(pager).queryByRole('link', { name: /Anteriores/ })).toBeNull()
  })

  it('en la página 2 el rango arranca en 51 y "Anteriores" vuelve a la 1', async () => {
    hasMore.value = false
    listMock.mockResolvedValue([row({ id: 'b1' }), row({ id: 'b2' })])
    // 52 = una página llena (50) + las 2 de acá: la última página, sin "Siguientes".
    countsMock.mockResolvedValue({ confirmed: 52 })

    render(await ReservasPage({ searchParams: Promise.resolve({ dia: 'pasados', pagina: '2' }) }))

    expect(screen.getByRole('status').textContent).toContain('51–52')
    const pager = screen.getByRole('navigation', { name: /Paginación/ })
    // Sin `?pagina=` — la página 1 es la URL limpia.
    expect(
      within(pager)
        .getByRole('link', { name: /Anteriores/ })
        .getAttribute('href'),
    ).toBe('/reservas?dia=pasados')
    expect(within(pager).queryByRole('link', { name: /Siguientes/ })).toBeNull()
  })

  it('un ?pagina basura no rompe: cae a la primera', async () => {
    listMock.mockResolvedValue([row({})])
    countsMock.mockResolvedValue({ confirmed: 1 })

    render(
      await ReservasPage({ searchParams: Promise.resolve({ dia: 'pasados', pagina: 'seis' }) }),
    )

    expect(listMock).toHaveBeenCalledWith('tenant-1', expect.anything(), expect.anything(), 0)
  })

  it('un ?pagina negativo tampoco: OFFSET nunca puede ser negativo', async () => {
    listMock.mockResolvedValue([row({})])
    countsMock.mockResolvedValue({ confirmed: 1 })

    render(await ReservasPage({ searchParams: Promise.resolve({ dia: 'pasados', pagina: '-4' }) }))

    expect(listMock).toHaveBeenCalledWith('tenant-1', expect.anything(), expect.anything(), 0)
  })
})
