...
- ✅ **🟡 jsonb double-encode en inserts crudos — AUDITADO + CERRADO** (2026-07-17). Clase: `${JSON.stringify(x)}::jsonb` en `tx.execute(sql...)` double-codifica bajo getDb Y getWorkerDb (`restoreJsonSerializers` re-aplica JSON.stringify al OID 3802; el customType de jsonb.ts solo cubre el query builder). **Barrido completo: exactamente 2 sitios buggy** — `payment.service.ts:199` (lockMpEvent) y `dunning.service.ts:50` (lockWebhook), ambos → `processed_webhooks.payload`. Impacto 🟢 LATENTE (getRecentWebhooks NO selecciona payload; write-only). Verificado empíricamente (probe: buggy→jsonb_typeof 'string' + `->>` NULL; fix→'object' + queryable). Fix: objeto crudo `${rawPayload}::jsonb`. Regression test `tests/integration/webhook-payload-jsonb.test.ts` (2 casos, honestidad probada). Safe (sin tocar): caja/reservas/dashboard actions (objeto crudo), tenant.service (`sql.json`), defaults literales, `set_config` (text).
