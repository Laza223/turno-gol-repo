...
--
-- ABONADOS.NOTES SE VA, PERO NO ACÁ. La columna es texto libre asociado a una
-- persona con nombre y teléfono: exactamente lo que D3 prohíbe, y existía desde
-- antes de la decisión. Decisión del dueño (2026-08-11): eliminarla. Verificado
-- contra producción: 0 filas en `abonados`, 0 con notas, 3 tenants vivos (todos
-- de prueba).
--
-- **El DROP va en la migración 075, en un release POSTERIOR**, por la regla de
-- `db-migrate.yml`: Vercel deploya por integración Git y no desde ese workflow,
-- así que el código nuevo y la migración corren en paralelo y no hay garantía de
-- orden. Sin required reviewers en el Environment `Production` (verificado), la
-- migración le gana al build de Next. Si el DROP viajara acá, durante esa
-- ventana el código VIEJO —que sigue arriba— rompería: `createAbonado` hace
-- `.insert(abonados).values({ …, notes })` y `pause/reactivate/cancel` hacen
-- `.returning()` sin argumentos, que Drizzle expande a todas las columnas del
-- schema viejo, `notes` incluida. `getAbonados` zafaría (usa `SELECT *`), pero
-- crear o pausar un turno fijo no.
--
-- Este release deja la columna huérfana: existe en la DB, ya no la escribe ni la
-- lee nadie. La 075 la dropea cuando este código ya esté arriba. Es el
-- expand-contract que `docs/operations/MIGRATIONS.md` pide para cualquier
-- rename/drop, y la razón por la que existe: pasó tres veces (048–051,
-- 059/061, 060–066).
...
-- Todo aditivo: revertir no pierde ningún dato preexistente.
