...
<body style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1e293b">
  ...
  <p>Tu cuenta <strong>${escapeHtml(data.tenantName)}</strong> está marcada para eliminación definitiva el <strong>${data.deletionDate}</strong>.</p>
  ...
  const text = `Tus datos se eliminan en ${data.daysRemaining} días\n\nHola ${data.ownerName},\n\nTu cuenta ${data.tenantName} se elimina el ${data.deletionDate}. Reactivá antes en: https://app.turnogol.app/settings/facturacion\n\n— El equipo de TurnoGol`
