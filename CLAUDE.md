...
- **Abonados sin saldo a favor (REVERTIDO, 2026-07-10)**: el sistema de crédito/saldo a favor copiado de ATC (`abonados.credit_balance`, `bookings.credit_applied`, `CashFlow` categoría `abonado_payment`) fue eliminado — modelo ATC descartado para fútbol. El abonado NO tiene saldo a favor ni precio mensual: solo `price_per_session` (precio por sesión). Distinto de `player_tenant_relationships.balance` (deuda por no-show), que se conserva intacto.
