...
## 2026-07-23 — Wave 2 D5 (continuación): retención por antigüedad IMPLEMENTADA (rama claude/practical-kilby-322fa9, stackeada sobre audit/data-d5)

**Decisión del dueño aprobada en D5 → código:** `purgeOldAuditLogs()` (24 meses por `created_at`) y `purgeOldNotifications()` (6 meses) en `data-retention-cleanup.worker.ts`, replicando el patrón `purgeProcessedWebhooks`: pasos GLOBALES pre-loop, try/catch propio cada uno (un fallo no aborta ni los otros pasos ni el wipe legal Ley 25.326). La purga es cross-tenant por antigüedad — el DELETE no filtra `tenant_id` a propósito (pool worker BYPASSRLS; `turnogol_app` tiene DELETE revocado sobre audit_logs, 008/037) e incluye filas system-level (`tenant_id IS NULL`).

**Test:** `tests/integration/retention-age-purges.test.ts` — vieja borrada (25m/7m), vigente conservada (23m/5m, prueba el boundary), re-corrida idempotente; aislamiento por prefijo en `action`/`trigger_event` sin `cleanupAll` (mismo patrón que processed-webhooks-retention). Gotcha del INSERT de notifications: el trigger `validate_notification_recipient` exige recipient real → `createTestPlayer` + tenant_id NULL.

**Gate:** typecheck ✅ / lint 0 errors (34 warnings pre-existentes) ✅ / retention-age-purges 2/2 ✅ / processed-webhooks-retention 1/1 ✅ / data-retention-cleanup 11/11 ✅.

**Docs:** fase-d5-infra-datos-report.md §REQUIERE INPUT item 3 marcado APROBADA+IMPLEMENTADA.

**Verificación adversarial (mismo día): RECHAZADO → 3 fixes → gate re-verificado verde.** El verificador (contexto fresco) reprodujo empíricamente que `turnogol_worker` tenía DELETE REVOCADO sobre `audit_logs` (migr. 038:68) → la purga nueva moría en prod con `permission denied` y el catch nuevo la escondía para siempre; local/CI enmascaran porque `getWorkerSql` cae a DATABASE_URL superusuario (4ta variante de la clase "local enmascara" de PR #30). Al barrer la clase se encontró PEOR: (a) `wipeTenant` (obligación legal Ley 25.326) también muere en prod — DELETE revocado sobre `audit_logs`/`daily_cash_closes` Y `SET LOCAL session_replication_role='replica'` es SUSET, `permission denied` bajo turnogol_worker (este último NO lo vio el verificador, salió al probar el fix); (b) el disclosure ARCO (`data-export/route.ts`) y doc18 decían "12 months" para audit_logs — con la purga real a 24 sería una afirmación de compliance falsa. Fixes: **migr. 057** (`GRANT DELETE ON audit_logs, daily_cash_closes TO turnogol_worker` — mínimo: UPDATE sigue revocado, turnogol_app intacto; espejo supabase/ manual) + tests de catálogo `has_table_privilege` + purga ejercitada bajo `SET LOCAL ROLE turnogol_worker` en retention-age-purges (trampa CI que cierra la clase) + disclosure/doc18/test ARCO actualizados a 24 meses/6 meses. **RESIDUO 🔴 (fuera de scope, escalado como task aparte):** `GRANT SET ON PARAMETER session_replication_role` es INAPLICABLE en Supabase (probado: `permission denied` incluso como `postgres`, que no es superusuario real) → el wipe legal completo sigue roto en prod hasta rediseñar ese paso (SECURITY DEFINER u orden explícito sin replica role). 🟡 backlog: DELETE por antigüedad sin índice de soporte (idx_audit_logs_tenant_created escanea todos los tenants; notifications hace seq scan) — aceptable hoy (semanal, prod chico), batching/índice parcial cuando haya volumen. Gate final: typecheck ✅ / lint 0 errors ✅ / retention-age-purges 4/4 + data-retention-cleanup 11/11 + processed-webhooks-retention 1/1 + arco-data-export 6/6 = 22/22 ✅ / purga verificada bajo rol real vía psql ✅. PENDIENTE dueño: aplicar 057 a prod.
