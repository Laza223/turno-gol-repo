...
- **Cobrar, en un toque** (`SlotChargeSection.tsx`): el método se elige en el desplegable de la propia línea (`SelectMenu`, Efectivo ya elegido) y **un botón dice el monto adentro**. Las pestañas Efectivo · Transferencia · MercadoPago que había arriba se fueron el 2026-09-17 (pedido del dueño): eran una segunda forma de elegir lo mismo y solo escribían el método de la primera línea. El panel no lleva recuadros internos: bloques "Cobro" y "Acciones" separados por un filete de 1 px — "Cobrar $ 15.000 y dar por jugado" / "…por adelantado" / "Cobrar $ 15.000" cuando se cobra el pendiente completo (`chargeCta` en `charge-copy.ts`). El monto va EN el botón porque es lo que evita tener que leer una tabla para saber qué se está por cobrar. El verbo es **siempre "cobrar"** (H017): "cerrar" está reservado y el título y el botón del mismo panel no pueden decir cosas distintas.
...
- Touch ≥ 44 px en todo: celda (columna de 48 px menos los 2 px de margen por lado), chevrons del día, "···", chip "Por cobrar hoy" y los campos de cobro del panel (monto y desplegable de método, `h-11`) — estos últimos bajan a 40 px recién en `md`, donde el puntero es un mouse.
