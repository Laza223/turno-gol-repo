...
| **3. Grilla mobile** | `BookingPopover` → `ui/popover.tsx` portaled con tap-to-toggle (revive iOS, conserva hover-intent desktop); fila compacta ≥44px; `BookingFormModal` gutter + fix clipping `phone-input` | BookingCard, BookingPopover, GridScroller, BookingFormModal | Medio | ✅ COMPLETADO 2026-07-07 (sin commitear) |
| **4. Touch-targets superficie jugador** | `AvailabilityGrid` a receta `h-11 md:h-9` (celdas Reservar, pills, nav día); `TenantHeader` chips; `FavoriteButton`; `ExplorarToolbar`; sticky `top-16→top-20` | AvailabilityGrid, TenantHeader, FavoriteButton, ExplorarToolbar, explorar/page | Bajo | ✅ COMPLETADO 2026-07-07 |
| **5. Touch-targets admin + primitives menores** | ConfirmDialog, date-picker (≥40px), dropdown-menu, ScheduleFields, image-uploader (24→44px), CourtList, AbonadoCreditLoader/DebtPayment/BookingActions/Charges, InviteStaffDialog stack, toast safe-area | ~12 archivos, clases puntuales | Bajo | ✅ COMPLETADO 2026-07-07 |
| **6. Polish player/business/onboarding** | perfil tabs (touch, NO scroll-tabs: son rutas), badge `whitespace-nowrap`, landing h1 clamp piso 38px, `para-complejos` `p-7 sm:p-11`, `CourtDraftCard` h-8→44px | 6 archivos | Bajo | ✅ COMPLETADO 2026-07-07 |
...
| 2026-07-07 | 3 | 🔴 #3 BookingPopover muerto en iOS + clipeado | `BookingPopover.tsx` → solo contenido (quitado el panel `absolute role=tooltip`); `BookingCard.tsx` envuelve la celda ocupada en `Popover`/`PopoverTrigger` (tap/click/Enter → abre en touch) + hover-intent 300ms/cierre 150ms con `cancelClose` al entrar al panel; `GridScroller` dropea props `popoverSide/Align` (Radix colisiona solo). Tests `booking-grid.test.tsx`: `role=tooltip`→`role=dialog`, `focus`→`click`, `+waitFor` cierre async | audit-verify 🟢; grilla-realtime chromium 1/1 (desktop intacto) |
| 2026-07-07 | 3 | 🟡 f) banda madrugada 32px (touch-targets rojo) | `GridScroller` gridTemplateRows banda `2rem`→`2.75rem`; `use-now-line.ts` `headerRem` sincronizado (2rem→2.75rem); comments en `MorningCollapseBand`/`use-grid-layout` | audit-verify 🟢 |
| 2026-07-07 | 3 | 🟡 f) BookingFormModal edge-to-edge + phone-input clip + `h-8.5` inválido | modal `w-full`→`w-[calc(100vw-2rem)]`; `phone-input.tsx` dropdown país hand-rolled → `ui/popover.tsx` portaled (no se clipea en el modal), `h-8.5`→`h-9` | audit-verify 🟢 |
| 2026-07-07 | 3 | 🟡 e) touch-targets residuales /grilla | `AdminThemeMenu` trigger `h-9 w-9`→`h-11 w-11 md:h-10`; `FirstBookingHint` "Entendido" `py-1`→`min-h-11 md:min-h-9`; `GridToolbar` densidad `min-w-11 sm:min-w-0` (icon-only <sm) | mobile suite 13/13 |
| 2026-07-07 | 3 | fix del pre-existente #1/#2 abonados-crud | spec apuntaba a `input[name=contactPhone]` (hidden del PhoneInput) → `#contactPhone` (input tel visible); cleanup por `+54 <nac>` (valor persistido) | abonados-crud chromium **4/4** |
| 2026-07-07 | — | **PRE-EXISTENTE reservas-crud** (stash-confirmado, NO es del Paso 3): 2 tests del dialog de cancelación — `¿Reembolsar la seña?` no aparece y orden de validación (`Ingresá un motivo` vs `Indicá quién cancela`). Flujo `reservas/[id]/BookingActions` NO tocado en este paso | fallan idéntico con y sin el diff; corresponde a fix de spec/código aparte |
| 2026-07-07 | 4-6 | 57 hallazgos touch/polish (audit de 3 agentes) | Scout paralelo (workflow, 8 clusters) verificó cada hallazgo vivo y devolvió edit exacto desktop-safe; apply script mecánico (exact-match + conteo de ocurrencias, replace-all solo en chevrons date-picker idénticos). P4: 13 edits (AvailabilityGrid celdas/pills/nav, TenantHeader, FavoriteButton, ExplorarToolbar, sticky top-20). P5: 37 edits (confirm-dialog, dropdown-menu, toast safe-area, date-picker celdas/chevrons/clear, ScheduleFields, image-uploader, CourtList, AbonadoCreditLoader, DebtPayment, BookingActions, BookingCharges, InviteStaffDialog stack). P6: 8 edits. Todo cascada `md:` (desktop intacto). ScheduleFields size=sm y trigger date-picker ya cumplían (scout: dead) | audit-verify 🟢 tras cada paso |
| 2026-07-07 | 4-6 | Review adversarial (workflow, review→verify) | 4 CONFIRMADOS, todos `touch-incomplete`, **0 desktop-regression** (cascada aguantó): input "Monto" quedó ~38px en AbonadoCreditLoader/DebtPayment/BookingCharges (hermano del select sin subir) + image-uploader capado en 36px cuando 44px entra (court 128×96, cover nunca reorder). Aplicados: Monto `h-11 md:h-9`, overlays `h-11 w-11 md:h-10 md:w-10` | audit-verify 🟢 |
| 2026-07-07 | — | **PRE-EXISTENTE canchas-crud + onboarding** (stash-confirmado): 4 tests fallan idéntico con y sin Pasos 4-6 — canchas-crud:124/192/325 (create/deactivate/rollback) + onboarding:85 (wizard completo). NO son regresión del refactor mobile | mobile 13/13 verde; desktop de superficies tocadas 26 passed |
