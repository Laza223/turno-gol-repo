...
- **Caja son 3 destinos colgados del `AdminHeaderSlot`, no 4 pestañas en el cuerpo**: `/caja` es **Vender** (catálogo, ticket, fiados abiertos — y nada agregado: ni totales ni diario), `/caja/cuentas` es el libro del dueño (cobrado hoy con su desglose por método, Deudas y Devolvés como dos listas que NUNCA se netean, movimientos del día) y `/caja/productos` es lo semanal (catálogo + informe desplegado al lado, ledger de stock plegado). `/caja/deudas`, `/caja/devoluciones` y `/caja/cantina` son redirects de compat; sus componentes y Server Actions siguen viviendo en esas carpetas. Spec: `docs/spec/design-system/pages/caja.md` v2.0 · Decisión: `docs/decisions/2026-09-12-rediseno-caja-tres-destinos.md`
