...
export function StepCourts({ existingCourts }: Props) {
  ...
  function addDraft() {
    setDrafts((prev) => {
      ...
      const n = existingCourts.length + prev.length + 1
      return [
        ...prev,
        {
          key: nextKey,
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {drafts.map((draft, i) => (
          ...
          >
            <legend className="sr-only">Cancha {i + 1}</legend>
        ...
        <Button type="button" variant="outline" className="w-full" onClick={addDraft}>
