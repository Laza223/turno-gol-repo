...
export default function DownloadReceiptButton({ fileName }: { fileName: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handlePrint}
      className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-white px-4 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-50 dark:border-white/10 dark:bg-white/[.04] dark:text-slate-200 dark:hover:bg-white/[.08]"
    >
