...
**El disparador se cumplió (2026-09-03)**: apareció en `claude/turnogol-8-bugs-mutation-da6095` (PR #265, campaña de mutación), una rama que no toca nada de `explorar` ni de mapas — su diff contra main no tiene un solo archivo de esa zona. Fallaron las MISMAS dos stories (`Composicion` y `HoverResaltaPinEnMapa`), en el mismo shard y solo en `light`. Confirmado como flake por el experimento que corresponde: re-correr el MISMO commit (`2e1de76f`) pasó en verde sin tocar una línea. Costó dos reruns y ~20 minutos de diagnóstico a alguien que no tenía forma de saber que era conocido. No apareció un tercer test del archivo, así que la prioridad queda donde está, pero ya no es "una racha en una rama".
