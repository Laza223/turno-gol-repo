...
- MercadoPago — **DOS aplicaciones, una por circuito de plata** (`docs/planning/2026-08-22-dos-apps-mercadopago.md`): la de **Suscripciones** cobra el plan SaaS con el token master del env (`MP_TURNOGOL_ACCESS_TOKEN`), la de **Checkout Pro** es la del OAuth por complejo para señas (`MP_CLIENT_ID`/`MP_CLIENT_SECRET`). MP deriva los scopes del PRODUCTO de la aplicación, y el permiso `payments:refunds` **no se puede dar por concedido**: la conexión de `complejo-titi` devolvía 403 en todos los intentos, y la de `complejo-elite-futbol` sí completó una devolución (refund `3199064441`, 2026-08-23 — sin confirmar contra la API). La diferencia entre las dos NO es la aplicación sino **de quién es la cuenta**: Elite está conectado con la cuenta master `381048203`, que es la **dueña** de las dos aplicaciones, y titi con la cuenta de un tercero (`1059888348`) — o sea que el único caso que funcionó es el que ningún cliente real va a ser. Se confirma con `pnpm tsx scripts/probe-mp-permissions.ts <tenantId>` sobre los dos. Un camino automático que anda para el dueño y falla para los clientes es peor que no tenerlo, así que **TurnoGol no reembolsa por API**: la devolución de la seña la hace el complejo y el sistema solo la registra, la lista en `/caja/devoluciones` y se la recuerda (el intento automático se eliminó, daba 403 siempre). Consecuencia: MP genera una clave de firma de webhook **por aplicación**, y las dos notifican al mismo buzón — `webhook-auth.ts` valida contra `MP_WEBHOOK_SECRET` (Suscripciones) **y** `MP_WEBHOOK_SECRET_CHECKOUT` (Checkout Pro)
...
- **`player_tenant_relationships.tags`** (`player_tag[]`, migr. 074, B12 / decisión v2 D3): etiquetas del complejo sobre una persona. **ENUM CERRADO de 5**: `gets_credit` (Se le fía) · `no_credit` (No fiar) · `group_organizer` (Organiza el grupo) · `agreed_price` (Tiene precio acordado) · `difficult` (Trato conflictivo); labels y ayudas en `src/modules/relationships/player-tags.ts`. El motivo del set cerrado es LEGAL, no de UI: **nunca texto libre sobre personas** (Ley 25.326, lo que un cliente puede leer ejerciendo derecho de acceso queda controlado en origen). Por eso `abonados.notes` se ELIMINÓ en la misma migración, y por eso la etiqueta vive en PTR y no en `players` (es del complejo, no viaja cross-tenant). Agregar una sexta es aditivo (`ALTER TYPE ... ADD VALUE`); **agregar texto libre no se concede: se reabre D3 con el dueño**. `[]` nunca NULL; `chk_ptr_tags_unique` (función `player_tags_are_unique`) prohíbe repetidos; `gets_credit` + `no_credit` juntas se rechazan en Zod. Escribe `setPlayerTags` (`ptr.service.ts`, query builder — el SQL crudo serializa mal un array de 1 elemento) y audita `player.tags_updated` con `before`/`after`.
...
- Planes SaaS (migr. 043; precios vigentes desde el `price_version` del 2026-08-07): Predio (1-3 canchas, $63.000/mes), Complejo (4-6, $99.000/mes), Estadio (7+, $129.000/mes); anual = 20% off. Los umbrales de canchas salen de `plans.max_courts`, no están escritos aparte. Sin límite de cantidad de staff. Página pública de planes: `/precios` (`src/app/(business)/precios/`, constantes en `plans-data.ts` a mantener en sync con la tabla `plans`).
...
- **Módulo Torneos** (`/torneos` + portal público `/[slug]/torneos`, migrs. 062–066, `docs/decisions/2026-07-24-torneos.md`): estaba fuera de scope (doc1 §6, P14.3, icebox §5h) y se REABRIÓ el 2026-07-24 por demanda. **Nace detrás del feature flag `tournaments`, global en `false`**: se prende por complejo con una fila de override en `feature_flags` (el flag se chequea en las páginas Y en cada Server Action, no solo en el menú). Tesis: **el torneo es un productor de reservas** — `bookings` tiene el exclusion constraint, así que todo lo que ocupe una cancha es una fila en `bookings`. Modelo de dos niveles: el torneo posee HORAS ENTERAS (`bookings.tournament_id` + `type='tournament'`, `price_snapshot=0`), y los partidos viven adentro de esas horas con su propio `starts_at` (fase 2) — por eso un relámpago de partidos de 25 min no rompe `SLOT_DURATION_MINUTES=60`. Calcado de abonados: `reserveTournamentSlots` = `insertBookingsForSlots` (skip-on-conflict, devuelve `conflicts`, no aborta la serie); `releaseTournamentSlots` = `cancelAbonado` (`DELETE ... WHERE date >= fromDate`). Fase 1 (hecha): `tournaments`, `tournament_teams`, `tournament_team_players`. Fase 2 (hecha, migr. 064): fixture y scheduler. Fase 3 (hecha, migr. 065): resultados, acta, tabla de posiciones, goleadores y disciplina — UNA tabla nueva (`tournament_match_events`, append-only: sin policy de UPDATE + REVOKE) y todo lo derivado se calcula al leer con un motor puro en `src/modules/tournaments/standings/`. Invariantes: un evento cuenta solo si su partido está en estado final; los goles a favor/en contra salen del MARCADOR y no de los eventos; el resultado de un walkover sale de `walkover_winner_team_id` y nunca del marcador. La 065 dropea los `chk_match_*_origin` de la 064, que hacían imposible el avance de llaves. Fase 4 (hecha, migr. 066): portal público y cobro de inscripción — CERO tablas nuevas. Un pago de inscripción ES una fila de `cash_flows` (`income`/`tournament` + `tournament_team_id`), así que entra a Caja y al cierre diario sin cableado extra y "cuánto pagó el equipo" es un SUM; `chk_cashflow_tournament_team` ata categoría ⟺ equipo en las DOS direcciones. El arancel se SNAPSHOTEA por equipo (`tournament_teams.inscription_fee`): subirlo a mitad de la inscripción no puede poner en deuda al que ya pagó. `registerInscriptionPayment` calca `addBookingChargeAction` (FOR UPDATE sobre el equipo antes de leer lo pagado, después el advisory lock diario — nunca al revés). Un cobro NO se deshace: borrar equipo/torneo con plata cobrada falla, para el que se bajó está `status='withdrawn'`. Portal en `/[slug]/torneos[/torneoSlug]` (ISR 300s, sin login): resuelve tenant por slug y abre `withTenantContext` — el aislamiento lo da la policy, `is_public` es un filtro de producto encima. Se publica: equipos, fixture, marcadores, tabla y goleadores. NO se publica: DNI, contacto, `player_id` ni el plantel completo. Planteles híbridos: nombres libres obligatorios, `player_id` opcional (el capitán sí se puede vincular). Guards: `requireAdminStaffAction` para crear/configurar, `requireOperatorStaff` para operar (el encargado toma horarios y anota equipos).
- **Módulo Personas** (`/jugadores`, cambio #9 + **B13**): la lista ÚNICA de personas del complejo. Desde B13 son **dos orígenes en un solo `UNION ALL`** (`listTenantClients` en `jugadores/queries.ts`): `kind:'player'` sale de `player_tenant_relationships ⋈ players`, y `kind:'contact'` se **deriva** de los `abonados` con `player_id NULL` agrupados por teléfono — la persona que el complejo conoce solo como nombre + teléfono porque es titular de un turno fijo. **No hay tabla nueva y es deliberado**: una tabla aislada arrastra el costo fijo de RLS+retención+isolation y, peor, deja dos fuentes de verdad para el mismo nombre; `abonados.contact_name` sigue siendo la única. Consecuencia asumida: **una persona sin cuenta NO puede tener etiquetas** (viven en PTR, que exige `player_id`) — vincularla las destraba, y eso es la decisión de producto, no un atajo. **Dos colas de teléfono, no una** (`src/modules/relationships/contact-identity.ts`): agrupar usa los últimos **10** dígitos (estricto: fusiona filas sin que nadie confirme, y cae al id de la fila si no hay ≥6 dígitos); sugerir usa los últimos **8** (laxo: lo confirma un humano, y es lo único que reconoce `0 11 15 …` como el mismo celular que `+54 9 11 …`). **Vinculación siempre manual** (decisión de fase v2 §1): `linkContactToPlayer`/`unlinkContactFromPlayer` (`contact-link.service.ts`) — mueve los abonados, reasigna solo las `bookings` SIN dueño de esos abonados, suma al `bookings_count` de PTR (contador incremental, no `COUNT(*)`) y NO toca `noshow_count` (nada de softbans retroactivos). El destino DEBE tener fila en PTR de ese tenant. Ficha = stats + softban + etiquetas (B12) + **Turnos fijos con "Desvincular"** + historial. Fuera por decisión de producto: invitados de `bookings.guest_name` (#10 cancelada) y deudores de `canteen_tabs`. `requireOperatorStaff()` (admin + manager). Pendiente de B10: el `LIMIT 200` sin cursor.
- **Caja y Cantina (rediseño 2026-07-22, migrs. 048–051)**: ítem de sidebar "Caja y Cantina", tabs por sub-ruta: `/caja` (movimientos, gastos, apertura/cierre), `/caja/cantina` (venta por ticket multi-ítem + fiados), `/caja/productos` (catálogo, reposición, mermas, reporte). Cantina sobre TABLAS REALES (`canteen_products` + ledger `stock_movements` append-only + `canteen_tabs` para fiados; módulo `src/modules/canteen/`) — el JSONB `tenants.settings.canteen_products` se backfilleó en 048 y se eliminó en 051. Venta = 1 `cash_flows` (income/`product_sale`) + N líneas en el ledger (`cash_flow_id`); fiado = stock sale al ENTREGAR, plata entra al COBRAR (`canteen_tabs`, `settleTab`); mermas/cortesías/consumo interno mueven stock sin tocar caja. Apertura de caja (`daily_cash_opens`, migr. 049): fondo inicial editable mientras el día está abierto; el cierre snapshotea `opening_cash`/`expected_cash` y `diff = contado − esperado` (cierres legacy: `expected_cash NULL`, nunca reinterpretar). Gastos con categorías (migr. 050): `merchandise`/`salaries`/`utilities`/`maintenance`/`other_expense` (`operating_expense` = legacy válido, la UI no lo ofrece). `cashflow_type`: `income` | `adjustment` | `expense`. Ingreso `tournament` (migr. 066, Torneos fase 4): SIEMPRE con `cash_flows.tournament_team_id` — `chk_cashflow_tournament_team` los ata en las dos direcciones, y la Server Action genérica de Caja NO lo ofrece (mismo criterio que con `bookingId`: la plata ligada a algo tiene un solo camino de entrada). Decisión: `docs/decisions/2026-07-22-caja-cantina-redesign.md` (la tabla `products` original se había dropeado en migr. 046 por dead code — `docs/decisions/2026-07-17-deprecate-products-table.md`; `canteen_products` la revive con otro shape y escritores reales).
