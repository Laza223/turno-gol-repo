...
| `MP_CLIENT_ID` / `MP_CLIENT_SECRET` | credenciales de la aplicación MercadoPago de **Checkout Pro** (la del OAuth de señas, no la de Suscripciones) | Vercel + Railway | Throw en boot (`env.ts:15-16`); `/api/status` los marca `down` si faltan (`src/app/api/status/route.ts:68`) |
| `MP_WEBHOOK_SECRET` | string ≥16 — clave de firma de la app de **Suscripciones** (la genera MP en Webhooks → Configurar notificaciones) | Vercel (lo consume el route handler) | Throw en boot prod (`env.ts:17`). Sin NINGUNA de las dos claves en prod la verificación **falla cerrado** → todo webhook responde 401 (`src/modules/payments/webhook-auth.ts`) |
| `MP_WEBHOOK_SECRET_CHECKOUT` | string ≥16 — clave de firma de la app de **Checkout Pro** (señas). MP genera una clave por aplicación y las dos notifican al mismo buzón | Vercel (no hace falta en Railway: la firma se valida en el route handler) | **Opcional en el schema a propósito** (`env.ts`), para que un deploy no se caiga entero por una clave todavía no cargada. La exige `launch:check` (`REQUIRED_ENV`). Sin ella, todo webhook de seña se rechaza con 401: el pago hecho en MP y la reserva colgada, visible sólo en el log de `rechazar()` |
...
- **Dónde se configura**: panel [MercadoPago Developers] → Tus integraciones → (tu aplicación) → **Webhooks → Configurar notificaciones** → "URL modo producción" `https://<dominio>/api/webhooks/mercadopago` → suscribir eventos → guardar → revelar la **clave secreta** que genera esa pantalla. Hay que hacerlo **en las DOS aplicaciones**: la de Suscripciones (eventos de suscripción → `MP_WEBHOOK_SECRET`) y la de Checkout Pro (evento Pagos → `MP_WEBHOOK_SECRET_CHECKOUT`). El `?tenant=` lo agrega el código como `notification_url` por operación (`src/modules/billing/billing.service.ts:131-133`), por eso `APP_URL` tiene que estar bien seteada.
