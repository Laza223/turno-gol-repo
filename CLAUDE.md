...
- **Módulo Jugadores** (`/jugadores`, cambio #9): vista admin de jugadores vinculados al complejo (vía `player_tenant_relationships`, NUNCA guests). Ficha = stats + indicador de softban activo (si `tenant_player_bans` tiene una fila vigente) + **etiquetas de cliente** (B12, ver `player_tenant_relationships.tags` arriba) + Abonados + historial. Sin cobro de deuda (ver softban arriba). Protegido con `requireOperatorStaff()` (admin + manager). OJO: `ClientesTabs.tsx` es la CÁSCARA de navegación, no la fusión de Clientes — `/jugadores` y `/abonados` siguen siendo dos listas de personas distintas, y un abonado con `player_id NULL` (nombre + teléfono, sin cuenta) no aparece en ninguna de las dos. Eso es B13, la segunda mitad de Fase 4.
