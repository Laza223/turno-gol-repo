...
3. **Retención `audit_logs`** (propuesta 24 meses) y **`notifications`** (propuesta 6 meses) — hoy crecen sin techo; solo se borran en erasure de tenant.
   - ✅ **APROBADA por el dueño (2026-07-23) e IMPLEMENTADA**: `purgeOldAuditLogs()` (24 meses por `created_at`) y `purgeOldNotifications()` (6 meses) en `data-retention-cleanup.worker.ts`, como pasos GLOBALES best-effort (try/catch propio cada uno, antes del early-return del loop per-tenant — mismo patrón que `purgeProcessedWebhooks`). La purga es cross-tenant por antigüedad (sin filtro de tenant, pool worker BYPASSRLS) e incluye filas system-level (`tenant_id IS NULL`). Test: `tests/integration/retention-age-purges.test.ts` (vieja borrada, vigente conservada, re-corrida idempotente, grants por catálogo, purga bajo `SET LOCAL ROLE turnogol_worker`; 4/4 ✅).
   - La verificación adversarial cazó que 038 le revocaba DELETE a `turnogol_worker` sobre `audit_logs` (y `daily_cash_closes`) → **migr. 057** otorga el DELETE mínimo (UPDATE sigue revocado; `turnogol_app` intacto). Disclosure ARCO (`data-export/route.ts`) y doc18 §7.1/§7.2 actualizados de "12 months" a 24 meses (+fila notifications 6 meses). **PENDIENTE dueño: aplicar 057 a prod.**
   - **RESIDUO 🔴 NUEVO (pre-existente, escalado aparte):** `wipeTenant` (erasure Ley 25.326) muere en prod bajo `turnogol_worker`: `SET LOCAL session_replication_role='replica'` es SUSET y el GRANT SET del parámetro es INAPLICABLE en Supabase (`permission denied` incluso como `postgres`). Requiere rediseño de ese paso (SECURITY DEFINER u orden explícito sin replica role). Local/CI no lo ven (DSN superusuario).
